...
**OPU** {% include cite.html id="2021_Cappelli_A_arxiv_ardacsg"%}. This defense relies on an analog layer implemented by an *Optical Processing Unit*. The later is a co-processor that multiplies any input vector by a fixed random matrix, using light scattering through a diffusive medium. The defense proposed by the authors expands the idea of obfuscated gradients to obfuscated parameters by physically implementing a fixed random projection followed by a non-linearity using an optical co-processor, where only the distribution of the random matrix entries is known and not their values.
